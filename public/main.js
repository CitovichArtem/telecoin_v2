...
let current = document.getElementById("GamePage");

let components = {
	container: current.createElement( "div" ),
	content: current.createElement( "div" ),
	wrapper: current.createElement( "div" ),
	cursor: current.createElement( "div" ),
	score: current.createElement( "div" ),
	gems: new Array(),
}
...
// start Game
// иф активное состояние геймпейдж, то инит игры
//
//
//
//
...
document.addEventListener('DOMContentLoaded', () => {
	const check3inRow = () => {
		let cur = document.getElementById('GamePage');
		if (cur){
			initGame();
		}else{
			console.log('no 333 in ROW, retrying...');
			setTimeout(check3inRow, 500); // Проверять каждые 500 мс
		}
	};
	check3inRow();
