...
let config = {
	...
	imagesCoin: ["images/coin/coin_1.png", "images/coin/coin_2.png", "images/coin/coin_3.png", "images/coin/coin_4.png", "images/coin/coin_5.png", "images/coin/coin_6.png"],
...
function initGame () {
	// Очистить предыдущие элементы игры, если они существуют
    clearGame();
...
function clearGame() {
    console.log('Очищаем предыдущие элементы игры...');

    // Удаление предыдущих элементов игры
    const oldGameContainer = document.getElementById('gameContainer');
    if (oldGameContainer) {
        console.log('удалили старую игру');
        $(oldGameContainer).remove();  // Используем jQuery для удаления элемента и его потомков
    }

    // Сброс состояния игры
    config.gameState = config.gameStates[0];

    // Остановка предыдущих таймеров и удаление обработчиков событий
...
$(document).ready(function() {
    ...
    // Проверяем, что мы на странице GamePage
    function checkGamePage() {
        if ($('#GamePage').length) {
            console.log('Мы на странице GamePage.');
            setupPlayButton();
        } else {
            console.log('Мы не на странице GamePage.');
        }
    }

    // Проверяем страницу при первоначальной загрузке
    checkGamePage();

    // Проверяем страницу при изменении истории
    $(window).on('popstate', checkGamePage);
});
...
function createPage() {
	...
	components.container.id = "gameContainer";
	...
	document.getElementById("GamePage").append(components.container);
}
...
function createGrid() {
	...
	for(let i = 0; i < config.countRows; i++ ) {
		for(let j = 0; j < config.countCols; j++ ) {
			...
			do{
				components.gems[i][j] = Math.floor(Math.random() * 6); // ! тут поменяла с 8 на 6
			} while( isStreak(i, j) );
...
function placeNewGems() {
	...
	for(let i = 0; i < config.countCols; i++ ) {
		if( components.gems[ 0 ][ i ] == -1 ) {
			components.gems[ 0 ][ i ] = Math.floor( Math.random() * 6 ); //! тут поменяла с 8 на 6
